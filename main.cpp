#include "raylib.h" //calling raylib libary
#include <math.h>
...
int main(void)
{
    // Initialization
    //--------------------------------------------------------------------------------------
    const int screenWidth = 800; //delcaring the size of the screen height and width this is a constant variable that cannot be changed
    const int screenHeight = 450;

    InitWindow(screenWidth, screenHeight, "Buzzing Bee's"); //intializing window and title of window
    ...
    while (!WindowShouldClose())    // Detect window close button or ESC key
    {
        ...
        if(IsMouseButtonDown(1)){
            map[(int)GetMouseX()/32][(int)GetMouseY()/32]=0;
        }
        
        // Here we slide down the blocks one step and add a new row
        slidedowntimer++;
        ...
        // Here we check if there are combinations that makes the blocks dissapear that are in that combination
        score();
        
        // This is the code that activates the block that moves towards the block destination
        if(startanim==false){
        ...
        // Animate the moving block(move it to the destination.)
        if(startanim){
            float an = getangle(animx,animy,animtargetx,animtargety);
            animx += cos(an)*8;
            animy += sin(an)*8;
        ...
        BeginDrawing();
            ...
            for(int x=0;x<25;x++){
                if(map[x][y]==1){
                    DrawRectangle(x*tileWidth,y*32,tileWidth,32,VIOLET);
                    DrawRectangle(x*tileWidth+1,y*32+1,tileWidth-2,32-2,VIOLET);
                }
                if(map[x][y]==2){
                    DrawRectangle(x*tileWidth,y*32,tileWidth,32,VIOLET);
                    DrawRectangle(x*tileWidth+1,y*32+1,tileWidth-2,32-2,MAROON);
                }
                if(map[x][y]==3){
                    DrawRectangle(x*tileWidth,y*32,tileWidth,32,VIOLET);
                    DrawRectangle(x*tileWidth+1,y*32+1,tileWidth-2,32-2,LIME);
                }
            ...
            if(animcolor==1){
                DrawRectangle(animx,animy,32,32,VIOLET);
                DrawRectangle(animx+1,animy+1,32-2,32-2,VIOLET);

            }
            if(animcolor==2){
                DrawRectangle(animx,animy,32,32,VIOLET);
                DrawRectangle(animx+1,animy+1,32-2,32-2,MAROON);
            }
            if(animcolor==3){
                DrawRectangle(animx,animy,32,32,VIOLET);
                DrawRectangle(animx+1,animy+1,32-2,32-2,LIME);
            }
